...
如果只是更新脚本，不修改现有节点配置、端口、Argo、WARP，可以直接进入安装菜单选择“更新 Litebox 脚本”。更新前会先检查 GitHub 项目是否有变化，并显示本地脚本 hash 和远程脚本 hash；如果远端脚本和本机一致，会提示当前已是最新，不会重复覆盖脚本，但会同步当前配置和端口跳跃规则。
...
`WARP 分流规则` 是可选分流模式，需要先安装或启用 WARP。分流菜单里可以选择“全部规则走 WARP”，也可以把下面 9 类服务分别切换为 `WARP` 或 `直连`；未切到 WARP 的服务继续按当前出口模式直连：
...
出口模式和分流规则是叠加关系。例如选择 `IPv4 优先` 后，如果 `OpenAI / ChatGPT -> WARP` 仍然开启，普通网站会走本机 IPv4，OpenAI / ChatGPT 会继续走 WARP。要关闭分流，需要进入 `WARP 分流规则管理` 把对应规则切回 `直连`。

WARP 目前只作为 IPv4 出口使用。若机器原生 IPv6 质量更好，建议选择 `IPv6 优先`，再按需开启少量 WARP 分流规则。
...
如果检测到 NAT / CGNAT IPv4，同时机器有公网 IPv6，安装时会让你选择节点入口：

- 使用 IPv6 入口，适合没有 IPv4 端口转发的 NAT 小鸡
- 使用 IPv4 入口，适合商家已经提供 IPv4 端口转发的 NAT 小鸡
- 手动输入入口地址，适合自定义域名或特殊映射

Litebox 会在 sing-box 配置里写入轻量 DNS，默认使用 IPv4 DNS，IPv6 优先/仅 IPv6 模式使用 IPv6 DNS，避免部分 NAT / IPv6 小鸡因为系统 DNS 不可达导致节点能连但网页打不开。
...
- 输入 `0`
- 跳跃端口最多允许 `50` 个
- 开启端口跳跃时会按需安装轻量 `iptables` 依赖，用于把跳跃端口重定向到实际服务端口
